...
class YunController extends Controller{
    ...
    public function DeleteClassification(){
        ...
        if(!empty($is_here)){
            ...
            //判断分类下是否有书签
            $is_marker = M('yunmarker');
            $has_marker = $is_marker->where($condition)->select();
            $markerDelete = 0;
            if(!empty($has_marker))
            {
            ...
            if($markerDelete==1)
            {
                ...
                if($marker->where($condition)->delete()){
                    //删除分类图片
                    $realPath = $_SERVER['DOCUMENT_ROOT'].$is_here['img'];
                    if(unlink($realPath))
                    {
                        $marker->commit();//成功则提交
                        ...
                        $result = array('status'=>2,'msg'=>'图片删除失败','img'=>$realPath);
                    }   
                }else{
                    $marker->rollback();//不成功，则回滚
                    $result = array('status'=>2,'msg'=>'分类删除失败','classification_id'=>$classification_id);
                }
            }else{
                    $marker->rollback();//不成功，则回滚
